...
public class 一和零_dp_官方题解 {
    ...
    public static void findMaxForm1(List<String> strs, int m, int n) {
//            如何考虑是否满足的问题
//            int i = (m < helper[0] || n < helper[1]) ? 0 : 1;

        for (String str : strs) {
            int[] helper = helper(str);
            for (int i = m; i >= helper[0]; i--) {
                for (int j = n; j >= helper[1]; j--) {
                    //如果不满足，直接返回
                    //假如有超出的
                    ints[i][j] = Math.max(ints[i][j], 1 + ints[i - helper[0]][j - helper[1]]);
                }
            }
            System.out.println(Arrays.deepToString(ints));
        }
